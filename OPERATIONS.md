...
- Run `pnpm ops:verify --id OPS-FF-MDM-001-01` before deployment to verify owner-label rendering, stable owner keys, unchanged account ownership, retained OIDC subjects, and equal maintenance access for two deterministic identities across HTTP, PostgreSQL, browser, and Compose fixtures. A passing `Operation OPS-FF-MDM-001-01 passed` result confirms that labels remain presentation-only. If it fails, restore labels by stable key, do not reassign accounts or OIDC identities, and roll back the application image if deployment has started.
